...
import zipfile
from scripts import x1, x2, x3,x4, x5
...
# ds_city_filter = ds.rio.write_crs("EPSG:4326")
...
# ds_filter = ds.sel(latitude=latitude, longitude=longitude, method='nearest')

graficos ,tab_infos, tab_map, tab_desc_indice = st.tabs(["Gráficos","Análise", "Mapa", "Descrição dos Índices"])

with graficos:

    st.write(f"### Índice: {indice}")
    st.write(f"#### Descrição: {desc_indices[indice]['Definição']}")
    st.markdown("---")
    ...
    for indice_choose in dict_indices.keys():
        if indice in dict_indices[indice_choose]:
            dict_func[indice_choose](ds,indice)
            st.markdown("---")
...
#     file_name = prepare_data(ds,'SP',indice,year_select,month_select_int,geojson_data,'MENSAL')
